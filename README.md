...
A synesthetic generative art engine that creates emotionally-adaptive compositions blending ASCII pattern art with mood-driven poetry. Core concept: every emotion has a visual signature and a linguistic voice. Features: (1) Emotion lexicon mapping 4 moods (joy, melancholy, wonder, fury) to curated word lists, symbol sets, visual palettes, and pattern behaviors; (2) Pattern art generator creating 12 distinct algorithmic patterns (wave, spiral, burst, orbit, shimmer, etc.) rendered through mood-specific character palettes; (3) Mood-driven poem generator with 8 line templates and emotional coherence; (4) Haiku generator with mood-themed vocabulary; (5) Synesthetic composition engine unifying visual art + haiku + poem in framed layouts; (6) Mood blend function showing emotional transitions between two moods. Uses seeded PRNG for reproducible output. Every function includes JSDoc with @example tags.
